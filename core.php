...
function wp_pagenavi( $args = array() ) {
	...
	switch ( intval( $options['style'] ) ) {
		...
		case 1:
			...
			if ( $start_page >= 2 && $pages_to_show < $total_pages ) {
				...
				$out .= $instance->get_single( 1, 'first', $first_text, '%TOTAL_PAGES%', $data );
			}

			// Previous
			if ( $paged > 1 && ! empty( $options['prev_text'] ) ) {
				$out .= $instance->get_single( $paged - 1, 'previouspostslink', $options['prev_text'], '%PAGE_NUMBER%', $data );
			}

			if ( $start_page >= 2 && $pages_to_show < $total_pages ) {
				if ( ! empty( $options['dotleft_text'] ) )
					$out .= $instance->get_extend( $options['dotleft_text'], $data );
			}
			...
			foreach ( $larger_pages_array as $larger_page ) {
				if ( $larger_page < ($start_page - $half_page_start) && $larger_page_start < $larger_page_to_show ) {
					$out .= $instance->get_single( $larger_page, 'smaller page', $options['page_text'], '%PAGE_NUMBER%', $data );
					$larger_page_start++;
				}
			}

			if ( $larger_page_start )
				$out .= $instance->get_extend( $options['dotright_text'], $data );
			...
			foreach ( range( $start_page, $end_page ) as $i ) {
				if ( $i == $paged && ! empty( $options['current_text'] ) ) {
					...
					$out .= $instance->get_single( $i, "page $timeline", $options['page_text'], '%PAGE_NUMBER%', $data );
				}
			}
			...
			foreach ( $larger_pages_array as $larger_page ) {
				if ( $larger_page > ($end_page + $half_page_end) && $larger_page_end < $larger_page_to_show ) {
					$larger_page_out .= $instance->get_single( $larger_page, 'larger page', $options['page_text'], '%PAGE_NUMBER%', $temp_data );
					$larger_page_end++;
				}
			}
			...
			if ( $paged < $total_pages && ! empty( $options['next_text'] ) )
				$out .= $instance->get_single( $paged + 1, 'nextpostslink', $options['next_text'], '%PAGE_NUMBER%', $data );

			if ( $end_page < $total_pages ) {
				// Last
				$out .= $instance->get_single( $total_pages, 'last', $options['last_text'], '%TOTAL_PAGES%', $data );
			}
		...
		case 2:
			...
			foreach ( range( 1, $total_pages ) as $i ) {
				...
				if ( $i == $paged ) {
					$current_page_text = str_replace( '%PAGE_NUMBER%', number_format_i18n( $i ), $options['current_text'] );
					$out .= '<option value="'. $url . '" selected="selected" class="current">'.$current_page_text."</option>\n";
				} else {
					$page_text = str_replace( '%PAGE_NUMBER%', number_format_i18n( $i ), $options['page_text'] );
					$out .= '<option value="'. $url . '">' . $page_text . "</option>\n";
				}
			}
	...
	$out = apply_filters( 'wp_pagenavi', $out, $data, $options );
...
class PageNavi_Call {
	...
	function get_single( $page, $class, $raw_text, $format = '%PAGE_NUMBER%', &$data = null ) {
		if ( empty( $raw_text ) )
			return '';

		$text = str_replace( $format, number_format_i18n( $page ), $raw_text );

		$url = esc_url( $this->get_url( $page ) );
		...
		if ( isset( $data ) ) {
			...
			/**
			 * If the link is special, reflect this in the data array.
			 */
			$special_links = array(
				'first' => 'first',
				'last' => 'last',
